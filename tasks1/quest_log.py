#!/usr/bin/env python3
"""
Quest Log - Gamified Task Management System
A command-line application that turns your tasks into epic quests with RPG mechanics.

Features:
- Store tasks (quests) with difficulty levels and rewards
- List active and completed quests
- Search through all quests
- Track XP, levels, and skills
...
class QuestLog:
    ...
    def search_quests(self):
        ...
        for quest in self.quests["active"]:
            if (keyword in quest["title"].lower() or 
                ...
                keyword in quest["skill"].lower() or
                keyword in quest["difficulty"].lower()):
        ...
        if not results:
            print(f"\n❌ No quests found matching '{keyword}'")
            return
        ...
        for quest, status in results:
            ...
            type_icon = {"main": "⚔️", "side": "📜", "daily": "🔄"}.get(quest["type"], "📝")
            
            print(f"{status_icon} {type_icon} {quest['title']}")
            print(f"   Status: {status.upper()} | Type: {quest['type'].title()} | Difficulty: {quest['difficulty']}")
            print(f"   Reward: {quest['xp_reward']} XP | Skill: {quest['skill'].title()}")
            if quest['description']:
                print(f"   Description: {quest['description']}")
            print()
    ...
    def main_menu(self):
        ...
        while True:
            ...
            print("5. 🔍 Search Quests")
            print("6. 👤 Character Sheet")
            print("7. 📝 Add Knowledge Note")
            print("8. 📚 View Knowledge Base")
            print("9. 💬 Chat with Quest Advisor")
            print("10. 🚪 Exit")
...
def main():
    ...
    print("\n" + "="*50)
    print("⚔️  QUEST LOG - Gamified Task Manager  ⚔️".center(50))
    print("="*50)
